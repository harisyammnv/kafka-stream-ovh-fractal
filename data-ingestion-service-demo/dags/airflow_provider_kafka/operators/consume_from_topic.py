...
class ConsumeFromTopicOperator(BaseOperator):
    ...
    def execute(self, context) -> Any:
        ...
        while messages_processed < self.max_messages:  # bool(True > 0) == True

            #msgs = consumer.consume(num_messages=batch_size, timeout=self.poll_timeout)
            msgs = consumer.poll(timeout=self.poll_timeout)
            ...
            if msgs:
                ...
                data.append(msgs.value())
                self.log.info(f"Processing msgs: {messages_processed}")
                if messages_processed % self.max_batch_size == 0:
                    apply_callable(data)
