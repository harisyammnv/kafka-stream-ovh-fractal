...
# defined the kafka topics,bootstrap and Mqtt client to access
# the data from mqtt protocol.
...
producer = AvroProducer(
    producer_config, default_key_schema=key_schema,
    default_value_schema=value_schema
)
mqtt_client = mqtt.Client("BridgeMQTT2Kafka")
# connecting the mqtt kafka produecr to kafka bridge .
mqtt_client.connect(MQTT_HOST)
# This function will used for producing the data on the kafka
# topics specify above.
def on_message(client, userdata, message):
    ...
    print("Send the message: " + msg_payload +
          f" to Kafka with topic {TOPIC}!")
    producer.flush()
